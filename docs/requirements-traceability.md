...
| 编排（ListVms / 批量电源） | `ListVms`、`StartVmGroup` / `StopVmGroup` | Hyper-V / PowerShell；[`platform_vm`](../crates/titan-vmm/src/platform_vm.rs) 路由 | [`platform_vm`](../crates/titan-vmm/src/platform_vm.rs), [`dispatch`](../apps/titan-host/src/serve/dispatch.rs) | Windows 集成；非 Windows 构建为 stub |
...
- **Capabilities**: built via [`Capabilities::from_host_runtime_probes`](../crates/titan-common/src/capabilities.rs) from [`host_runtime_probes::probe_host_runtime_blocking`](../apps/titan-host/src/host_runtime_probes.rs) at `serve` startup (`hyperv_spoof_host`, `kernel_driver_ipc`, vision flags — honest defaults until wired).
