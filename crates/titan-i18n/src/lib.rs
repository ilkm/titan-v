...
#[must_use]
pub fn hp_batch_no_plans(lang: UiLang) -> String {
    match lang {
        UiLang::En => "No VMs to create: add an explicit VM or a VM group.".into(),
        UiLang::Zh => "没有可创建的虚拟机：请添加「显式 VM」或「VM 组」".into(),
    }
}

#[must_use]
pub fn hp_provision_start_banner(lang: UiLang, dry_run: bool, n: usize) -> String {
    match lang {
        UiLang::En => {
        ...
        UiLang::Zh => {
            ...
            format!("开始{phase} — 共 {n} 台")
        }
    }
}

#[must_use]
pub fn hp_provision_done_ok(lang: UiLang) -> String {
    match lang {
        UiLang::En => "Batch job finished.".into(),
        UiLang::Zh => "批量任务结束".into(),
    }
}

#[must_use]
pub fn hp_provision_done_err(lang: UiLang, err: &str) -> String {
    match lang {
        UiLang::En => format!("Batch failed: {err}"),
        UiLang::Zh => format!("批量失败: {err}"),
    }
}
