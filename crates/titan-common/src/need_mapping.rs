...
//! 5. `cargo fmt` / `clippy -D warnings` / `cargo test --workspace` 通过；非 Windows 上 `titan-vmm::platform_vm` 仅为 stub（空列表 / 明确未实现），**不**宣称 Hyper-V 或 guest 物理内存能力。
//!
...
//! | ListVms / 域电源入口 | `titan-vmm::platform_vm`（Windows → `hyperv`） |
