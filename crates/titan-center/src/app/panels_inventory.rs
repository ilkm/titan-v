...
impl CenterApp {
    ...
    fn panel_window_management_split(&mut self, ui: &mut egui::Ui) {
    ...
    pub(super) fn panel_window_management(&mut self, ui: &mut egui::Ui) {
        ...
        self.panel_window_management_split(ui);
    }
    ...
    fn inventory_vm_host_line(&self) -> String {
        self.endpoints
    ...
    fn paint_inventory_vm_thumb(ui: &mut egui::Ui, row: &VmBrief, w: f32) {
    ...
    fn paint_inventory_vm_captions(&self, ui: &mut egui::Ui, row: &VmBrief, host_line: &str) {
        ui.add_space(6.0);
        ui.label(
    ...
    fn paint_inventory_vm_tile(&self, ui: &mut egui::Ui, row: &VmBrief, host_line: &str, w: f32) {
        ui.set_width(w);
        Self::paint_inventory_vm_thumb(ui, row, w);
        self.paint_inventory_vm_captions(ui, row, host_line);
    }

    fn panel_vm_youtube_cards(&self, ui: &mut egui::Ui) {
        let host_line = self.inventory_vm_host_line();
        ui.horizontal_wrapped(|ui| {
            ...
            for row in self.inventory_slice() {
                ...
                ui.vertical(|ui| {
                    self.paint_inventory_vm_tile(ui, row, &host_line, w);
                });
                ui.add_space(4.0);
            }
        });
    ...
    fn panel_host_disk_volume_row(ui: &mut egui::Ui, v: &DiskVolume) {
        ui.horizontal(|ui| {
            ui.label(
            ...
            egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    for v in self.disk_volumes_slice() {
                        Self::panel_host_disk_volume_row(ui, v);
                    }
                });
        });
    }
